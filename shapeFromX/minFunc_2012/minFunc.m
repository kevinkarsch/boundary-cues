function [x_struct,f,exitflag,output] = minFunc(funObj,x0_struct,options,varargin)
...
[verbose,verboseI,debug,doPlot,maxFunEvals,maxIter,optTol,progTol,method,...
    ...
    LS_type,LS_interp,LS_multi,checkGrad, F_STREAK, F_PERCENT, F_MINITERS] = ...
...
[x0, options.TEMPLATE] = struct2vector(x0_struct);
...
if method < NEWTON
  if numDiff
    x_struct = vector2struct(x, options.TEMPLATE);
    [f, g] = findgrad(x_struct, funObj, 10^-5, varargin{:});
  else
    x_struct = vector2struct(x, options.TEMPLATE);
    [f,g_struct] = feval(funObj,x_struct,varargin{:});
    g = struct2vector(g_struct);
  end
  computeHessian = 0;
else
...
for i = 1:maxIter
    ...
    elseif LS_type == 1 % Find Point satisfying Wolfe conditions
        ...
        if computeHessian
            [t,f,g,LSfunEvals,H] = WolfeLineSearch(x,t,d,f,g,gtd,c1,c2,LS_interp,LS_multi,25,progTol,debug,doPlot,LS_saveHessianComp,numDiff,funObj,varargin{:});
        else
            [t,f,g,LSfunEvals] = WolfeLineSearch(x,t,d,f,g,gtd,c1,c2,LS_interp,LS_multi,25,progTol,debug,doPlot,1,numDiff,funObj,options,varargin{:});
        end
        funEvals = funEvals + LSfunEvals;
        x = x + t*d;
...
%   end
  ...
  if verboseI
    %       fprintf('%02d | %+1.4e |', funEvals*funEvalMultiplier - last_fun_count, f);
    fprintf('%02d | ', funEvals*funEvalMultiplier - last_fun_count);
    print_count = print_count + 1;
    last_fun_count = funEvals*funEvalMultiplier;
    if print_count == 4
      fprintf('%s/eval\n', time2human(etime(clock, start_time) / (funEvals*funEvalMultiplier)));
      print_count = 0;
    end
  ...
  if (length(trace.fval) > max(F_STREAK, F_MINITERS)) && all(abs((trace.fval(end) - trace.fval((end - F_STREAK) : (end-1)))./trace.fval(end)) < (F_PERCENT/100))
    exitflag=1;
    msg = 'F_STREAK conditions hit';
    break;
  end
...
x_struct = vector2struct(x, options.TEMPLATE);
if verboseI
  fprintf([time2human(etime(clock, start_time)), '\n']);
end
