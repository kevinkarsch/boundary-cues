...
function [labeldata hf] = imlabel(im, labeldata)%imgid, VOCopts)
    hf=figure(1);
        ...
        if strcmp(opt, 'Ext Bd') == 1 || strcmp(opt, 'Ext Bd2') == 1
            while(1)
                ...
                elseif k == 'u'
                    ...
                    if strcmp(opt, 'Ext Bd') == 1
                        labeldata.extbd = [labeldata.extbd; x y 0];
                        disp('added a smooth ext boundary point');
                    else
                        labeldata.extbd = [labeldata.extbd; x y 1];
                        disp('added a sharp ext boundary point');
                    end
                    disp([x y]);
                    drawextbd(labeldata.extbd);
                end
            end
...
%                     disp('exiting Exterior Bd');
%                     plotscene(labeldata);
%                     break;
%                 elseif k == 'q'
%                     if ~isempty(pts) 
%                         labeldata.extbd{numel(labeldata.extbd)+1} = pts; 
...
%                         pts(end,:) = [];
%                         plotscene(labeldata);
%                         if(~isempty(pts)) drawextbd(pts); end
%                     end
%                 else
%                     pts = [pts; [x y]];
%                     drawextbd(pts);
