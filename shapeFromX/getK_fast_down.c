...
void mexFunction(int nargout, mxArray *varargout[], int nargin, const mxArray *varargin[])
{
  ...
  dK_dims[0] = (int)(Z_sz[0]/2);
  dK_dims[1] = (int)(Z_sz[1]/2);
  dK_dims[2] = 6;
  ...
  K_mat = mxCreateDoubleMatrix(dK_dims[0], dK_dims[1], mxREAL);
  K = mxGetPr(K_mat);
  varargout[0] = K_mat;
  
  for(i1 = 0; i1 < Z_sz[0]; i1=i1+2){
    for(j1 = 0; j1 < Z_sz[1]; j1=j1+2){
