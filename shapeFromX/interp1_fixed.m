...
if size(LUT_F,2) > size(LUT_F,1)
  LUT_F = LUT_F';
end
F = zeros(size(X));
F(within_idx) = LUT_F(below_bin) .* V2 + LUT_F(above_bin) .* V1;
...
if QUADRATIC_EXTRAPOLATION
  F(above_idx) = above_F + (above_dF*bin_width)*delta + (0.5*above_ddF*bin_width)*delta.^2;
else
  F(above_idx) = above_F + (above_dF*bin_width)*delta;
end
...
delta = X(below_idx);
if QUADRATIC_EXTRAPOLATION
  F(below_idx) = below_F + (below_dF*bin_width)*delta + (0.5*below_ddF*bin_width)*delta.^2;
else
  F(below_idx) = below_F + (below_dF*bin_width)*delta;
end
